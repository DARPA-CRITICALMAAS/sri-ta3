...
class CMALitModule(LightningModule):
    ...
    def calibrated_forward(self,
        ...
        """
        return self.net(x, pca_matrix) / torch.tensor(self.hparams.temperature).to(x)
    ...
    def model_step(
        self, batch: Tuple[torch.Tensor, torch.Tensor, torch.Tensor], calibrated: bool = False
        ...
        """
        x, y, pca_matrix = batch
        pca_matrix = pca_matrix.detach().half() if len(pca_matrix.shape) != 1 else None

        if calibrated:
            logits = self.calibrated_forward(x, pca_matrix)
        else:
            logits = self.forward(x, pca_matrix)
    ...
    def predict_step(self, batch: Tuple[torch.Tensor, torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        if self.hparams.extract_attributions:
            ...
            if len(batch[4].shape)!=1:
                attribution = ig.attribute(batch[0].requires_grad_(), additional_forward_args=batch[4].half(), n_steps=12).mean(dim=(-1,-2)).detach()
            else:
        ...
        preds = torch.sigmoid(
            self.calibrated_forward(
                ...
                batch[4].tile((self.hparams.mc_samples,1,1)).half() if len(batch[4].shape)!=1 else None
