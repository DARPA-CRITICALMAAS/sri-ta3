...
class BinaryTemperatureScaling(nn.Module):
    ...
    def calibrate(self, datamodule, val_fraction):
        ...
        labels_list = []
        with torch.no_grad():
            for inputs, label, _, _, cols, rows, pca_matrix in islice(val_loader, batch_limit):
                ...
                logits = self.model(inputs, cols, rows, pca_matrix).detach()
    ...
    def search_threshold(self, max_metric, datamodule, val_fraction):
        ...
        with torch.no_grad():
            for inputs, label, _, _, cols, rows, pca_matrix in islice(val_loader, batch_limit):
                inputs = inputs.to(dtype=torch.float32, device=self.model.device)
                pca_matrix = pca_matrix.to(dtype=torch.float32, device=self.model.device) if len(pca_matrix.shape) != 1 else None
                logit = torch.sigmoid(self.model.calibrated_forward(inputs, cols, rows, pca_matrix))
