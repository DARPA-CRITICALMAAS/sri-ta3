#!/bin/bash

# loads the project variables (namespace, etc.)
source project_vars.sh

# repo for images should be the same as the namespace
export REPO="${NAMESPACE}"

# runs the container locally with access to GPU and proper volume mounts
docker run --rm -it --gpus all -p 8888:8888 \
    --name=cmaas-ta3-local-${DUSER} \
    -v /dev/shm:/dev/shm \
    -v ${DATA_PATH}:/workspace/data \
    -v ${LOGS_PATH}:/workspace/logs \
    -v ${PROJ_ROOT}/${SRC_PATH}:/workspace/${SRC_PATH} \
    -e NAMESPACE -e SYSTEM_NAME -e SYSTEM_VERSION -e SYSTEM_DESCRIPTION -e MODEL_NAME -e MODEL_VERSION \
    -e CDR_TOKEN -e CDR_HOST -e CALLBACK_URL \
    -e NGROK_AUTHTOKEN -e NGROK_SECRET -e NGROK_PORT \
    -e JOB_TAG -e REPO_HOST -e DUSER -e WANDB_API_KEY \
    ${REPO_HOST}/${REPO}:${SYSTEM_NAME}-${DUSER}-v${SYSTEM_VERSION} bash
